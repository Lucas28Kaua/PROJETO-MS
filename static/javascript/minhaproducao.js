...
async function pegarValoresDoFormulario(event) {
    ...
    const url = currentEditingCard 
        ? `https://sistemamscred.com.br/propostas/editar/${idProposta}` 
        : "https://sistemamscred.com.br/propostas/criar";
...
async function carregarPropostasDoBanco() {
    ...
    try {
        ...
        const response = await fetch(`https://sistemamscred.com.br/propostas?usuario_id=${usuarioId}`);
...
async function excluirCard(button) {
    ...
    if (confirm(`Tem certeza que deseja excluir a proposta de ${dados.nome}?`)) {
        try {
            const response = await fetch(`https://sistemamscred.com.br/propostas/excluir/${idProposta}`, {
                method: 'DELETE'
            });
